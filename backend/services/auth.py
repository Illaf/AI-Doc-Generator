...
SessonLocal=sessionmaker(bind=engine,autoflush=False,autocommit=False)
...
def github_connect(current_user: User = Depends(get_current_user)):
    # Debug prints so we see the REAL error if any
    print("DEBUG -> Entered /github/connect")

    GITHUB_CLIENT_ID = os.getenv("GITHUB_CLIENT_ID")
    GITHUB_REDIRECT_URI = os.getenv("GITHUB_REDIRECT_URL")

    print("DEBUG CLIENT:", GITHUB_CLIENT_ID)
    print("DEBUG REDIRECT:", GITHUB_REDIRECT_URI)
...
async def github_callback(code:Optional[str]=None,request:Request=None,db:Session =Depends(get_db),current_user:User=Depends(get_current_user)):
    if code is None:
    ...
    gh_account=db.query(GitHubAccount).filter(GitHubAccount.user_id == current_user.id).first()
    if gh_account:
        ...
        db.add(gh_account)
    db.commit()
    return {"detail": "GitHub account linked", "username": gh_user.get("login")}

@auth_router.get("/github/repos")
async def get_github_repos(db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    async with httpx.AsyncClient() as client:
        ...
        resp.raise_for_status()
        repos = resp.json()
    out=[{"id": r.get("id"), "name": r.get("name"), "full_name": r.get("full_name"), "private": r.get("private"), "html_url": r.get("html_url")} for r in repos]
    return {"repos":out}
